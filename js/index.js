...
$("#navPop").on("click", function (e) {
  ...
  $("nav").animate({ left: "0px" }, 700, function () {
    $(child).animate({ top: "0" }, 500, function (params) {
      $(child)
        .next()
        .animate({ top: "0" }, 500, function (params) {
          $(child)
            .next()
            .next()
            .animate({ top: "0" }, 500, function (params) {
...
async function getFirstData() {
  ...
  const firstData = [
    52977, 53065, 53060, 53069, 53026, 52978, 52948, 52844, 52845, 52971, 53013,
    52804, 53027, 52785, 52929, 52769, 52802, 52906, 53028, 52887, 52980, 53006,
    52963, 52931, 52811,
  ];
  ...
  for (let i = 0; i < firstData.length; i++) {
    const respons = await fetch(
      `https://www.themealdb.com/api/json/v1/1/lookup.php?i=${firstData[i]}`
    );
...
async function test(div) {
  ...
  closeNavBar();
  const respons = await fetch(
    `https://www.themealdb.com/api/json/v1/1/search.php?s=${div.lastElementChild.firstElementChild.innerHTML}`
  );
  const result = await respons.json();
...
async function fullCategory() {
  ...
  for (let i = 0; i < data.categories.length; i++) {
    categoryData(
      ...
      getFirst20Words(data.categories[i].strCategoryDescription),
...
async function mainIngredients(main) {
  ...
  try {
    ...
    firstReload.classList.remove("d-none");
    const respons = await fetch(
      `https://www.themealdb.com/api/json/v1/1/filter.php?c=${main.lastElementChild.firstElementChild.firstElementChild.innerHTML}`
    );
    if (!respons.ok) {
      throw new Error("Failed to fetch data");
    }
...
async function getAreaMeals(country) {
  ...
  firstReload.classList.remove("d-none");
  const response = await fetch(
    `https://www.themealdb.com/api/json/v1/1/filter.php?a=${country.lastElementChild.innerHTML}`
  );
  const data = await response.json();
  console.log(data);
...
async function displayIngredientDetails(data) {
  ...
  firstReload.classList.remove("d-none");
  const response = await fetch(
    `https://www.themealdb.com/api/json/v1/1/filter.php?i=${data.firstElementChild.nextElementSibling.innerHTML}`
  );
